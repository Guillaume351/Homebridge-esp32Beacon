...
export class BeaconHandler {
  ...
  public static singleRoomPerMac = true;
  ...
  public static getTriggerDetectionThreshold(beaconName: string) {
    const beaconsSettings: Array<BeaconSetting> =
      BeaconPlatform.instance.config.devices;
...
export class Beacon {
  ...
  constructor(public readonly accessory: PlatformAccessory) {}
  ...
  public addTrack(beaconName: string, deviceMac: string, rssi: number) {
    if (rssi > BeaconHandler.getTriggerDetectionThreshold(beaconName)) {
      // TODO : replace by a better system
      this.trackHistory.push(deviceMac);
      setTimeout(() => {
        this.removeOldestTrack();
      }, 10000);

      if (BeaconHandler.singleRoomPerMac) {
        // if an other beacon contains the same mac address, remove it from his track history
        BeaconHandler.beacons.forEach((beacon) => {
          if (beacon !== this) {
            beacon.removeDevice(deviceMac); // TODO: we should ignore timeouts pending
          }
        });
      }

      this.updateState();
    } else if (rssi > BeaconHandler.getMaintainDetectionThreshold(beaconName)) {
